...
// +----------------------------------------------------------------------
// | Author: bao <1352760448@qq.com>
// +----------------------------------------------------------------------
// | bao在 2022-2-26 进行修订，通过Mysql保存验证码而不是Session以更好的支持API访问
...
class Captcha
{
    ...
    {
        //var_dump('验证码：',strtoupper($code), $id);
        $key = $this->authcode($this->seKey, $id);  //这里加密了
        ...
        //var_dump("失败了", $this->authcode(strtoupper($code), $id));
        //var_dump("失败了", $secode['code']);
    ...
    public function create(string $id, $captcha = false)
    {
        ...
        //var_dump("我生成的验证码222333=",$captcha, $id);
        $code    = $this->authcode($captcha, $id);
        //var_dump("我生成的验证码2=",$code );
        Db::name('captcha')
            ->insert([
                'key'        => $key,
                'code'       => $code,
